...
if __name__ == "__main__":
    ...
    return_queue = deque(maxlen=args.stats_window_size)
    length_queue = deque(maxlen=args.stats_window_size)

    progress = tqdm.trange(1, num_updates + 1)
        ...
        for step in range(0, args.num_steps):
            ...
            for item in info:
                if "final_info" in item.keys() and "episode" in item["final_info"].keys():
                    ...
                    writer.add_scalar("charts/episodic_length", np.mean(length_queue), global_step)
                    break
